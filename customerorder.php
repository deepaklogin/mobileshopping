	...
	if (mysqli_fetch_array($sq4) > 0) {
		...
		while ($r4 = mysqli_fetch_array($sq4)) {
			...
			if ($status == "1") {
				...
				if ($sq2) {
					if (mysqli_fetch_array($sq2) > 0) {
						...
						while ($r2 = mysqli_fetch_array($sq2)) {
							echo '<div class="col-xs-6 mx-auto"><img src="' . $r2['mobile_image'] . '" width="460px"></div>';
				...
				echo '<a class="btn btn-danger btn-sm form-control" href="cancelorder.php?bookphoneid=' . $bookphoneid . '" >Cancel Order</a>';
				echo '</div>';
			}
		}
	}
